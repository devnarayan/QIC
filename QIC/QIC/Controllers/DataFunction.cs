...
namespace SoftSwitch.Web.Code
{
    public class DataFunctions
    {
        ...
        public string GetmaxID(string fld, string tbl)
        {
            string str_connection = Convert.ToString(obj_connectionstr.ConnectionString());
            SqlConnection con = new SqlConnection(str_connection);
            SqlCommand cc = new SqlCommand("select Isnull(max(" + fld + "),0) from " + tbl, con);
        ...
        public int ExecuteProcedureInOut(String Procedure, Hashtable HT,string outparam)
        {
            string str_connection = Convert.ToString(obj_connectionstr.ConnectionString());
            SqlConnection con = new SqlConnection(str_connection);
            SqlCommand cmd = new SqlCommand(Procedure, con);
            ...
            try
            {
                ...
                cmd.Parameters["@"+ outparam].Direction = ParameterDirection.InputOutput;
                con.Open();
                int i = cmd.ExecuteNonQuery();
                int ii = Convert.ToInt32(cmd.Parameters["@"+ outparam].Value);
                con.Close();
                con.Dispose();
                cmd.Dispose();
                if (i > 0) return ii;
